...
(function (global) {
  ...
  var CONFIG = {
    ...
    // 🆕 Smart Context Tracking (성능 최적화 유지)
    contextTracking: {
      enabled: true,  // 🔧 기본값: 활성화 (항상 기록)
      maxBreadcrumbs: 20,  // 최근 20개만 유지
      recentTimeWindow: 30000,  // 최근 30초 이내만 유효
      ...
      smartFilters: {
        ...
        trackOnlyErrors: false      // false: 모든 행동 추적
      },
  ...
  var ContextManager = {
    ...
    init: function() {
      ...
      if (!this.enabled) {
        console.log("[ContextManager] ⏸️ Context tracking이 비활성화되어 있습니다.");
        return;
      }
      
      console.log("[ContextManager] ✅ Context tracking 활성화 (항상 기록)");
      console.log("  → 최근 " + this.limit + "개 이벤트를 메모리에 유지");
      console.log("  → 30초 이상 지난 이벤트는 자동 삭제");
      console.log("  → 에러 발생 시 즉시 분석에 활용");
  ...
  var ErrorAnalyzer = {
    ...
    handleError: function(errObj) {
      ...
      // Context 정보 가져오기
      var behaviorLog = ContextManager.getHistoryText(20);
      ...
      // 에러 발생 직전 동작 확인
      var recentActions = ContextManager.getRecentByCategory('ACTION', 3);
      
      console.log("%c📋 수집된 Context 정보:", "color:#2196F3");
      console.log("  • 행동 기록:", ContextManager.breadcrumbs.length + "개");
      console.log("  • 최근 입력:", recentInputs.length + "개");
      console.log("  • 최근 서브미션:", recentSubmissions.length + "개");
      if (duplicatePatterns) {
        console.log("  • 중복 패턴 감지됨!");
      }
      ...
      if (hasContext) {
        ...
        if (recentActions.length > 0) {
          contextSection += "[최근 동작]\n";
          for (var m = 0; m < recentActions.length; m++) {
            contextSection += "  " + recentActions[m].time + " - " + 
                            recentActions[m].message + "\n";
          }
          contextSection += "\n";
        }
        ...
        contextSection += "[전체 행동 타임라인 (최근 20개)]\n" + behaviorLog + "\n";
      } else {
        contextSection = "\n💡 참고: 사용자 행동 기록이 없습니다.\n" +
                        "   Context 추적을 활성화하려면:\n" +
                        "   ContextManager.toggle(true)\n";
      }
      ...
      var prompt = "=== 에러 정보 ===\n" +
                   ...
                   contextSection + "\n\n" +
                   "=== 분석 요청 ===\n" +
                   "위의 에러 정보를 분석하여 한국어로 답변하세요.\n" +
                   (hasContext ? 
                    "🔍 사용자 행동 Context를 **반드시 참고**하여 구체적으로 분석하세요.\n" +
                    "특히 [최근 동작], [최근 입력값], [최근 서브미션]에서 에러 원인을 찾으세요.\n\n" : 
                    "\n") +
                   "**필수 출력 양식** (한국어로만 작성):\n\n" +
                   "1. 에러 원인:\n" +
                   "   (한 문장으로 핵심 원인 설명)\n\n" +
                   "2. 왜 발생했나:\n" +
                   (hasContext ? 
                    "   (Context 정보를 참고하여 구체적으로 2줄 이내로 설명)\n\n" :
                    "   (에러 발생 이유를 2줄 이내로 설명)\n\n") +
                   "3. 해결 방법:\n" +
                   "   ```javascript\n" +
                   "   // ❌ 문제가 되는 코드\n" +
  ...
  global.contextHelp = function() {
    ...
    console.log("  ContextManager.getRecentByCategory('ACTION', 5)");
    ...
    console.log("");
    console.log("%c💡 팁: 기본적으로 항상 활성화되어 있습니다!", "color: #4CAF50");
  };

  global.chatHelp = function() {
    console.log("%c=== AI Assistant v2.5 도움말 ===", "color: #2196F3; font-weight: bold; font-size: 16px");
    console.log("");
    console.log("%c✓ 자동 에러 분석 (Context-Aware)", "color: #FF9800; font-weight: bold");
    console.log("  JavaScript 에러 발생 시 자동으로 분석합니다.");
    console.log("  에러 발생 전의 사용자 행동(클릭, 입력, 서브미션)을");
    console.log("  자동으로 추적하여 정확한 원인을 분석합니다.");
    console.log("  중복 패턴도 자동 감지합니다.");
    console.log("");
    ...
    console.log("");
    console.log("%c✓ Context 명령어", "color: #FF9800; font-weight: bold");
    console.log("  contextHelp()           - Context Manager 상세 도움말");
    console.log("  ContextManager.getHistoryText() - 행동 기록 확인");
    console.log("  ContextManager.clear()  - 타임라인 초기화");
    console.log("");
    console.log("%c🎯 v2.5 핵심 특징:", "color: #4CAF50; font-weight: bold");
    console.log("  • 항상 사용자 행동 추적 (최근 20개, 30초)");
    console.log("  • 에러 발생 전 행동 분석");
    console.log("  • 중복 입력 패턴 자동 감지");
    console.log("  • 스마트 필터링 (무의미한 클릭 제외)");
    console.log("");
    console.log("%c💾 모델 정보:", "color: #9E9E9E");
    console.log("  현재 모델: " + CONFIG.modelName);
    console.log("  권장 모델: qwen-1.5b (한국어 품질 우수)");
  };
  ...
  console.log("%c🤖 AI Assistant v2.5 로드 완료", "color: #ffffff; background: #2196F3; font-weight: bold; font-size: 14px; padding: 5px");
  console.log("%c" + "=".repeat(70), "color: #2196F3; font-weight: bold");
  console.log("");
  console.log("%c주요 기능:", "color: #4CAF50; font-weight: bold");
  console.log("  ✓ 자동 에러 분석 (Context-Aware)");
  console.log("  ✓ API 검색: search('검색어')");
  console.log("  ✓ AI 채팅: chat('질문')");
  console.log("  ✓ Context 관리: contextHelp()");
  console.log("");
  console.log("%c🎯 v2.5 핵심 특징:", "color: #FF9800; font-weight: bold");
  console.log("  • 항상 사용자 행동 추적 (에러 발생 전부터)");
  console.log("  • 최근 20개 이벤트, 30초 이내만 유지");
  console.log("  • 중복 패턴 자동 감지");
  console.log("  • 스마트 필터링 (무의미한 클릭 제외)");
  console.log("");
