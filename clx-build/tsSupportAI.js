...
(function (global) {
  ...
  var AISupport = {
    ...
    /**
     * console.error 훅을 통해 전달된 에러 처리
     * @param {Array} args - console.error 인자 배열
     */
    captureConsoleError: function (args, severity) {
      if (!args || !args.length) {
        return;
      }
      ...
      if (extractedError) {
        this.handleError({
          ...
          message: extractedError.message || "Unknown console error",
          stack: extractedError.stack,
          consoleArgs: args,
          source: severity ? "console." + severity : "console.error",
      ...
      var first = args[0];
      var msg = typeof first === "string" ? first : (function () {
        try {
      ...
      this.handleError({
        message: msg || "Unknown console error",
        consoleArgs: args,
        source: severity ? "console." + severity : "console.error",
        severity: severity || "error",
        timestamp: new Date().toISOString()
  ...
  /**
   * window.addEventListener("error", ...) - 캡처 단계에서 우선 처리
   */
  if (window.addEventListener) {
    window.addEventListener("error", function (event) {
      // event.error가 없으면 message 기반으로 처리
      var errObj;
      if (event.error) {
        errObj = {
          ...
          source: event.filename || (event.target && event.target.src),
          lineno: event.lineno,
          colno: event.colno,
          timestamp: new Date().toISOString(),
          via: "capture-error-event"
        };
      } else {
        errObj = {
  ...
  /**
   * 모든 DOM/윈도우 이벤트 리스너를 try-catch로 래핑
   * (EventTarget.prototype.addEventListener 훅킹)
   */
  (function () {
    var EventTargetPolyfill = window.EventTarget || window.Node;
    if (!EventTargetPolyfill || EventTargetPolyfill.prototype.__AISupportPatched) {
      return;
    }

    var nativeAdd = EventTargetPolyfill.prototype.addEventListener;
    var nativeRemove = EventTargetPolyfill.prototype.removeEventListener;
    var handlerMap = new WeakMap(); // 원본 -> 래핑 함수
    ...
    function wrapHandler(handler, type) {
      ...
      wrapped = function () {
        ...
        if (type === "error" && eventObj && window.AISupport && window.AISupport.analyze) {
          try {
            var payload = eventObj.error || {
              message: eventObj.message,
              filename: eventObj.filename,
              lineno: eventObj.lineno,
              colno: eventObj.colno
            };
            window.AISupport.analyze(payload);
          } catch (errEarly) {
            console.warn("[AI Error Assistant] error 이벤트 사전 분석 실패:", errEarly);
          }
        }
        ...
        try {
          ...
          try {
            if (window.AISupport && window.AISupport.analyze) {
              window.AISupport.analyze(err);
            }
          } catch (analyzeErr) {
            console.warn("[AI Error Assistant] 이벤트 훅 분석 실패:", analyzeErr);
          }
          throw err; // 원래 에러 유지
        }
      };
    ...
    EventTargetPolyfill.prototype.addEventListener = function (type, listener, options) {
      return nativeAdd.call(this, type, wrapHandler(listener, type), options);
    };
    ...
    EventTargetPolyfill.prototype.__AISupportPatched = true;
    console.log("[AI Error Assistant] EventTarget.addEventListener 훅킹 완료");
  })();

  /**
   * console.error 훅킹 (엔진보다 먼저 그리고 이후에도 재적용)
   */
  (function () {
    ...
    function wrapConsoleMethod(method) {
      ...
      if (console[method].__AISupportWrapped) {
        return;
      }

      var original = console[method];

      function wrappedConsoleMethod() {
        var args = Array.prototype.slice.call(arguments);

        if (!reentryGuard) {
          reentryGuard = true;
          try {
            if (window.AISupport && window.AISupport.captureConsoleError) {
              window.AISupport.captureConsoleError(args, method);
            }
          } catch (hookErr) {
            try {
              original.call(console, "[AI Error Assistant] console." + method + " 훅 오류:", hookErr);
            } catch (nestedErr) {
              // ignore
            }
          }
      ...
      wrappedConsoleMethod.__AISupportWrapped = true;
      wrappedConsoleMethod.__AISupportOriginal = original;
      console[method] = wrappedConsoleMethod;
      console.log("[AI Error Assistant] console." + method + " 훅킹 적용");
    }

    function scheduleHooks() {
      methods.forEach(function (name) {
        wrapConsoleMethod(name);
      });
    }

    var hookDelays = [0, 50, 250, 1000, 2000];
    hookDelays.forEach(function (delay) {
      setTimeout(scheduleHooks, delay);
    });

    setInterval(function () {
      methods.forEach(function (method) {
        if (!console || !console[method] || console[method].__AISupportWrapped) {
          return;
        }
